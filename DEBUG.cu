...
__device__ void BarrettRedc(uint32_t Z[],uint32_t p[],uint32_t miu[],uint32_t res[],uint32_t qHat[],uint32_t r[]){
    // compute q_hat
...
__global__ void func1(uint32_t Z[],uint32_t P[],uint32_t res[],uint32_t qHat[],uint32_t r[]){
    
    uint32_t miu[3] =   {0x708a9a6b,0xb1dbb0f0,0x1};
    BarrettRedc(Z,P,miu,res,qHat,r);
    return ;
}
...
int main(){
    ...
    uint32_t a[3] = {0x11223344,0x55667788,0x1};
    uint32_t qHat[3] = {0x0,0x0,0x0};
    uint32_t r[3] = {0x0,0x0,0x0};
    uint32_t b[2] = {0x55ff7788,0x99aabbcc};
    uint32_t *d_Z,*d_P,*d_res,*d_qHat,*d_r;
    ...
    cudaMalloc((void**)&d_qHat , sizeof(uint32_t) * 3);
    cudaMalloc((void**)&d_r , sizeof(uint32_t) * 3);
    cudaMemcpy( d_P,P , sizeof(uint32_t) * 2, cudaMemcpyHostToDevice);
    cudaMemcpy( d_Z,Z , sizeof(uint32_t) * 4, cudaMemcpyHostToDevice);
    func1<<<1,32>>>(d_Z,d_P,d_res,d_qHat,d_r);
    cudaMemcpy( res, d_res, sizeof(uint32_t) * 2, cudaMemcpyDeviceToHost);
    cudaMemcpy( r, d_r, sizeof(uint32_t) * 3, cudaMemcpyDeviceToHost);
    cudaMemcpy( qHat, d_qHat, sizeof(uint32_t) * 3, cudaMemcpyDeviceToHost);
    for(int i = 0;i<2;i++){
        cout<<std::hex<<res[i]<<endl;
    }
    cout<<endl<<"QHAT:"<<endl;
    for(int i = 0;i<3;i++){
        cout<<std::hex<<qHat[i]<<endl;
    }
    cout<<endl<<"R:"<<endl;
    for(int i = 0;i<3;i++){
        cout<<std::hex<<r[i]<<endl;
    }
    return 0;
}